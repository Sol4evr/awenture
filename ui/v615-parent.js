...
function syncExpandedState(){document.querySelectorAll('.aw-subskill-details').forEach(details=>details.querySelector('summary')?.setAttribute('aria-expanded',String(details.open)))}
function wireSubskills(){document.querySelectorAll('.aw-subskill-details').forEach(details=>{if(details.dataset.awCollapsible==='1')return;const summary=details.querySelector('summary');if(!summary)return;details.dataset.awCollapsible='1';summary.setAttribute('aria-expanded',String(details.open));details.addEventListener('toggle',()=>{if(details.open)document.querySelectorAll('.aw-subskill-details[open]').forEach(other=>{if(other!==details)other.removeAttribute('open')});syncExpandedState()})})}
...
window.__AW_PARENT_UI={subskillsCollapsedByDefault:true,singleOpenAccordion:true,visualSubjectCues:true,statusPolling:true};
})();
