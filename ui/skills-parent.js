...
function subjectBlock(c,subject){const rows=c.skills.filter(s=>s.subject===subject),covered=rows.filter(s=>s.questionCount>0).length,domains=[...new Set(rows.map(s=>s.domain))];return `<details class="aw-subject-panel aw-skills-subject" data-aw-skills-subject="${esc(subject)}"><summary role="button" aria-expanded="false"><span class="aw-subject-summary-leading"><span class="aw-subject-copy"><b>${esc(subject==='Mathematics'?'Maths':subject)}</b><small>${covered} of ${rows.length} framework skills have question coverage</small></span></span><strong class="aw-subject-score">${rows.length?Math.round(covered/rows.length*100):0}%</strong><span class="aw-subject-chevron" aria-hidden="true">⌄</span></summary><div class="aw-skills-domains">${domains.map(domain=>{const d=rows.filter(s=>s.domain===domain),dc=d.filter(s=>s.questionCount>0).length;return `<div class="aw-skill-domain"><div><b>${esc(domain)}</b><small>${dc}/${d.length} covered</small></div><ul>${d.map(s=>`<li class="${s.questionCount?'covered':'gap'}"><span>${esc(s.label)}</span><small>${s.questionCount?`${s.questionCount} question${s.questionCount===1?'':'s'}`:'Top-up gap'}</small></li>`).join('')}</ul></div>`}).join('')}</div></details>`}
function mount(){
  ...
  const card=title.closest('.card');if(!card||card.dataset[FLAG]==='1'||!framework())return;
  const c=coverage();if(!c)return;
  const anchor=card.querySelector('.aw-inventory');if(!anchor)return;
  ...
  section.innerHTML=`<div class="aw-section-title"><div><span>Skills coverage</span><small>AWenture framework · Year ${esc(c.grade)} · loads only in Parent View</small></div><b>${pct}%</b></div><p class="aw-skills-note">${c.coveredSkills} of ${c.totalSkills} framework skills currently have mapped practice questions. Gaps are prioritised for governed Question Factory top-ups; coverage does not unlock progression.</p><div class="aw-subject-grid aw-skills-grid">${framework().subjects.map(s=>subjectBlock(c,s)).join('')}</div>`;
  anchor.parentNode.insertBefore(section,anchor);card.dataset[FLAG]='1';
}
...
mount();
window.__AW_SKILLS_PARENT={coverage,focus:skillFocus,mount,progressionEnabled:false};
})();
