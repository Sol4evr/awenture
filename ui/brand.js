(()=>{
'use strict';
const SRC='/awenture-logo-192.png?v=61520';
...
function sync(){const landing=!![...document.querySelectorAll('h1')].find(x=>x.textContent.includes('Hello Alistair'));document.querySelectorAll('.brand').forEach(x=>{mark(x,false);x.classList.toggle('aw-brand-landing',landing)});document.querySelectorAll('.test-home').forEach(x=>mark(x,true));document.querySelectorAll('.testbrand').forEach(x=>{if(x.querySelector('.aw-brand-logo'))return;const b=x.querySelector('button');if(b)mark(b,true);else{x.insertAdjacentHTML('afterbegin',`<img class="aw-brand-logo compact aw-brand-static" src="${SRC}" alt="AWenture" width="28" height="28" decoding="async">`)}})}
...
window.__AW_BRAND={asset:SRC,version:'aw-brand-1.3'};
})();