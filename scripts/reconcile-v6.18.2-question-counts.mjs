...
let revokedCrossSubject=0,filenameVerified=0,sourceIdentityVerified=0;
for(const [src,e] of Object.entries(qa.papers||{})){
  ...
  if(e.questionCountVerified&&supportPath&&!supportMatchesSubject(src,supportPath,p)){
    e.questionCount=null;e.questionCountVerified=false;e.method=null;e.evidence='question-count QA pending: rejected cross-subject support evidence';revokedCrossSubject++;
  }
...
console.log(JSON.stringify({release:'6.18.2',strictQuestionCountReconciliation:'PASS',verified:qa.summary.verified,pending:qa.summary.pending,revokedCrossSubject,filenameVerified,sourceIdentityVerified,unresolvedByStageSubject}));
