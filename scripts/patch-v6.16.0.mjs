...
const insightReplacements=[
  ...
  ["function backendPayload(req){return {requestKey:req.requestId,sourceRelease:req.release,bankSize:req.bankSize,targetUnseenPerSubject:TOPUP_TARGET,unseenBySubject:req.unseen,requiredBySubject:req.requested,focusSubskills:Object.fromEntries(TOPUP_SUBJECTS.map(s=>[s,req.weakest[s].map(x=>x.skill)])),qaPolicy:req.qualityPolicy}}","function backendPayload(req){return {requestKey:req.requestId,sourceRelease:req.release,bankSize:req.bankSize,targetUnseenPerSubject:TOPUP_TARGET,unseenBySubject:Object.fromEntries(TOPUP_SUBJECTS.map(s=>[s,req.unseen[s]])),requiredBySubject:req.requested,focusSubskills:Object.fromEntries(TOPUP_SUBJECTS.map(s=>[s,req.weakest[s].map(x=>x.skill)])),qaPolicy:req.qualityPolicy}}"],
