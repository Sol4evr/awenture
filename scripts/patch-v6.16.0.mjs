...
const speechReplacement="let speechUtterance=null;\nfunction stopRead(){if('speechSynthesis'in window)speechSynthesis.cancel();speechUtterance=null;speaking=false;}\nfunction read(){if(!('speechSynthesis'in window)||!('SpeechSynthesisUtterance'in window)||!sess)return;const synth=window.speechSynthesis;if(speaking){stopRead();return render()}const q=sess.items[sess.i],spoken=q.subject==='Spelling'&&q.audioWord?[q.audioWord,q.audioSentence,q.audioWord]:[q.stimulus,q.question,...q.options.map((o,i)=>L[i]+'. '+o)],u=new SpeechSynthesisUtterance(spoken.filter(Boolean).join('. ')),vs=synth.getVoices(),v=vs.find(v=>/Karen|Matilda|Samantha/i.test(v.name))||vs.find(v=>/^en-AU/i.test(v.lang))||vs.find(v=>/^en-/i.test(v.lang))||vs[0];u.lang='en-AU';if(v)u.voice=v;u.rate=q.subject==='Spelling'?.82:.92;u.pitch=1.04;const done=()=>{if(speechUtterance!==u)return;speechUtterance=null;speaking=false;render()};u.onend=done;u.onerror=done;speechUtterance=u;speaking=true;if(synth.paused)synth.resume();synth.speak(u);render();}";
