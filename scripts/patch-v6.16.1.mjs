...
insights=insights.replace(syncMarker,`function enableRollingSubskills(){requestAnimationFrame(()=>{for(const label of document.querySelectorAll('.aw-subskill-spectrum .aw-spectrum-copy b')){let span=label.querySelector(':scope>span');if(!span){const text=label.textContent||'';label.textContent='';span=document.createElement('span');span.textContent=text;label.appendChild(span)}label.classList.remove('aw-roll-label');label.style.removeProperty('--aw-roll-distance');label.style.removeProperty('--aw-roll-duration');if(label.clientWidth<=0)continue;const overflow=Math.max(0,span.scrollWidth-label.clientWidth);if(overflow>4){label.classList.add('aw-roll-label');label.style.setProperty('--aw-roll-distance',(-overflow-10)+'px');label.style.setProperty('--aw-roll-duration',Math.max(7,Math.min(14,5+overflow/28))+'s')}}})}\ndocument.addEventListener('toggle',e=>{if(e.target?.matches?.('.aw-subskill-details')&&e.target.open)enableRollingSubskills()},true);window.addEventListener('resize',enableRollingSubskills,{passive:true});\nlet queued=false;function sync(){transformHome();transformParent();enableRollingSubskills()}`);
