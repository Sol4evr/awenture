...
// Diagnostic inventory of Y3/Y4 answer resources only. This is deliberately NOT an automatic
// promotion channel: it surfaces candidate same-subject/year evidence for manual Y2-standard review.
const answerKeyRows=[];for(const abs of walk(path.join(root,'source','original-icas'))){const rel=path.relative(root,abs).split(path.sep).join('/');if(!/^source\/original-icas\/year(?:3|4)\//i.test(rel)||!/answer|solution|marking/i.test(rel))continue;try{const doc=await pdfjs.getDocument({data:new Uint8Array(fs.readFileSync(abs)),disableWorker:true,isEvalSupported:false,useSystemFonts:true}).promise;for(let n=1;n<=doc.numPages;n++){const text=await pageText(doc,n),nums=answerNumbers(text),seq=seqStats(nums);if(!seq||seq.coverage<.6||seq.tail<2)continue;const years=[...new Set([...text.matchAll(/(?:19|20)\d{2}/g)].map(m=>Number(m[0])))];answerKeyRows.push({sourcePath:rel,page:n,years,sequence:seq,text:compact(text,650)})}try{doc.destroy()}catch(_){}}catch(_){}}
const out={version:'aw-icas-cover-count-diagnostic-v3-y3y4',generatedAt:new Date().toISOString(),pendingIcas:Object.entries(qa.papers||{}).filter(([s,e])=>!e.questionCountVerified&&['icas-y3','icas-y4'].includes(meta.get(s)?.stage)).length,candidates:rows.length,byCount,rows,answerKeyRows};
fs.writeFileSync(path.join(root,'dist/stage-papers/icas-cover-count-diagnostic.json'),JSON.stringify(out,null,2)+'\n');
console.log(JSON.stringify({release:'6.18.2',icasCoverCountDiagnostic:'PASS',version:'v3-y3-y4-answer-key-inventory',pendingIcas:out.pendingIcas,candidates:rows.length,byCount,answerKeyCandidates:answerKeyRows.length,answerKeyRows:answerKeyRows.map(r=>({sourcePath:r.sourcePath,page:r.page,years:r.years,sequence:r.sequence,text:r.text}))}));
