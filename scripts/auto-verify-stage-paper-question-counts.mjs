...
function numberedCandidates(lines){
  ...
  for(const line0 of lines){const line=line0.trim();
    ...
    const all=[...line.matchAll(/\bQuestion\s+(\d{1,3})\b/ig)];for(const x of all){const n=clampCount(x[1]);if(n)nums.push(n)}
  }
  return nums;
}
function sequenceInference(nums,min){
  ...
  for(let n=max;n>=min;n--){
    ...
    if(coverage>=0.78&&tail>=2){best={n,coverage:Number(coverage.toFixed(3)),present};break}
  }
  return best;
}
function chooseEvidence(e,p){
  ...
  if(totals.length){
    ...
    return {n:chosen,method:'document-total',evidence:totals.filter(x=>x.n===chosen).slice(0,3)};
  }
  ...
  if(ranges.length){
    ...
    if(hasTail||multiple)return {n:maxEnd,method:'question-range-end',evidence:ranges.filter(x=>x.end===maxEnd).slice(0,3)};
  }
  return null;
}

const result={version:'aw-stage-question-count-2',generatedAt:new Date().toISOString(),policy:{paperSpecific:true,learnerPagesOnly:true,noSubjectDefaultForVerifiedAttempt:true,writingSingleResponse:true,sectionRangeAware:true,plausibilityFloor:true},papers:{},summary:{total:0,verified:0,pending:0,writing:0,totalDeclaration:0,rangeEnd:0,sequence:0},unresolved:[]};
for(const st of Object.values(catalog.stages||{}))for(const p of st.papers||[]){
  ...
  try{
    if(p.subject==='Writing'){
      ...
      const evidence=countEvidence(all),chosen=chooseEvidence(evidence,p);
      if(chosen){
        entry.questionCount=chosen.n;entry.questionCountVerified=true;entry.method=chosen.method;entry.evidence=chosen.evidence;
        if(chosen.method==='document-total')result.summary.totalDeclaration++;else result.summary.rangeEnd++;
      }else{
        const inf=sequenceInference(numberedCandidates(all),plausibleMinimum(p));
        ...
        else entry.evidence='question-count QA pending: no high-confidence total, terminal range, or complete numbered sequence';
      }
