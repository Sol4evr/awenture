...
const out={version:'aw-stage-count-target-diagnostic-v2',generatedAt:new Date().toISOString(),targets:rows};fs.writeFileSync(path.join(root,'dist/stage-papers/count-targets.json'),JSON.stringify(out,null,2)+'\n');
console.log(JSON.stringify({release:'6.18.2',countTargetDiagnostic:'PASS',version:'v2',targets:rows.map(r=>({sourcePath:r.sourcePath,sha256:r.sha256,totalPages:r.totalPages,pages:r.pages.map(p=>({page:p.page,answer:p.answer,results:p.results,cover:p.cover,answerSequence:p.answerSequence,denominators:p.denominators}))}))}));
