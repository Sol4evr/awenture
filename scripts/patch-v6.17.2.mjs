...
const newMakePath=`function makePath(api){const m=api.model(),wrap=document.createElement('div');wrap.className='aw-functional-path aw-learning-line';wrap.dataset.awLearningPath='1';const labels={'icas-y2':'ICAS Y2','icas-y3':'ICAS Y3','naplan-y3':'NAPLAN Y3','icas-y4':'ICAS Y4','oc-prep':'OC'};for(const [i,s] of m.config.stages.entries()){const on=m.progress.unlocked.includes(s.id),cur=m.progress.current===s.id,b=document.createElement('button');b.type='button';b.className='aw-path-step aw-stage-step'+(on?' unlocked':' locked')+(cur?' active current':'');b.disabled=!on;b.dataset.stage=s.id;b.setAttribute('aria-current',cur?'step':'false');b.setAttribute('aria-label',(labels[s.id]||s.label)+(cur?' current stage':on?' unlocked':' locked'));b.innerHTML='<span class="aw-path-dot">'+(on?(cur?'●':'✓'):'🔒')+'</span><span class="aw-path-label">'+(labels[s.id]||s.label)+'</span>'+(i<m.config.stages.length-1?'<span class="aw-path-rail" aria-hidden="true"></span>':'');if(on)b.addEventListener('click',()=>{api.setCurrent(s.id);renderPath()});wrap.appendChild(b)}return wrap}`;
...
const newCss=`.aw-functional-path.aw-learning-line{display:flex;align-items:flex-start;gap:0;overflow-x:auto;padding:8px 4px 12px;scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch}.aw-functional-path .aw-path-step{position:relative;flex:1 0 112px;min-width:112px;display:flex;flex-direction:column;align-items:center;gap:7px;padding:0 8px;border:0;background:transparent;color:inherit;overflow:visible;scroll-snap-align:center}.aw-functional-path .aw-path-dot{position:relative;z-index:2;width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:#fff;border:2px solid #c9d1df;font-size:15px;line-height:1}.aw-functional-path .aw-path-step.unlocked .aw-path-dot{border-color:var(--primary);color:var(--primary);background:#eef2ff}.aw-functional-path .aw-path-step.current .aw-path-dot{background:var(--primary);color:#fff;border-color:var(--primary)}.aw-functional-path .aw-path-step.locked{opacity:.58}.aw-functional-path .aw-path-label{max-width:108px;text-align:center;font-size:12px;font-weight:800;line-height:1.2;white-space:normal;overflow-wrap:normal}.aw-functional-path .aw-path-rail{position:absolute;z-index:1;top:16px;left:calc(50% + 17px);width:calc(100% - 34px);height:2px;background:#d8dee8}.aw-functional-path .aw-path-step.unlocked .aw-path-rail{background:linear-gradient(90deg,var(--primary),#d8dee8)}.aw-functional-path .aw-path-step:enabled{cursor:pointer}.aw-functional-path .aw-path-step:enabled:focus-visible .aw-path-dot{outline:3px solid rgba(49,88,212,.25);outline-offset:3px}@media(max-width:620px){.aw-functional-path .aw-path-step{flex-basis:102px;min-width:102px;padding:0 5px}.aw-functional-path .aw-path-label{max-width:98px;font-size:11px}}`;
