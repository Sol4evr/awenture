...
for(const f of ['premium.css','practice-flow.css','home-insights.css','challenge.css'])fs.copyFileSync(path.join(root,'ui',f),path.join(root,'dist',f));
for(const f of ['premium.js','insights.js','challenge.js'])fs.copyFileSync(path.join(root,'ui',f),path.join(root,'dist',f));
fs.writeFileSync(path.join(root,'dist/index.html'),html);
console.log(JSON.stringify({release,baselineRelease:m.release,baselineSha256:sha,baselineBytes:baseline.length,coreBank:126,approvedExpansion:expansion.length,totalCoreBank:126+expansion.length,bonusBank:bonus.length,bonusVisuals:bonus.filter(q=>q.kind==='visual').length,visualIds:visualIds.length,contentCorrections:Object.keys(corrections).length,answerMix,uiModules:['premium-v1','practice-flow-v1','home-insights-v1','challenge-v1'],output:'dist'}));
