...
const js=`(()=>{'use strict';
const STAGE_LABELS={'icas-y3':'ICAS Year 3','naplan-y3':'NAPLAN Year 3','icas-y4':'ICAS Year 4','oc-prep':'Opportunity Class'};
let catalog=null,renderQueued=false;
...
function baseGrid(card){const h=qsa('h1',card).find(x=>(x.textContent||'').trim()==='Subject tests');return h&&qs('.grid',card)}
...
console.log(JSON.stringify({release:'6.18.0',feature:'stage-linked-historical-paper-library',activated:catalog.summary.activated,pending:catalog.summary.pending,stages:Object.keys(catalog.stages)}));
