...
const direct=`const bindPager=(sel,fn)=>{const b=qs(sel);if(!b)return;let pointerAt=0;b.addEventListener('pointerdown',e=>{if(b.disabled)return;pointerAt=Date.now();e.preventDefault();e.stopPropagation();fn()});b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(b.disabled||Date.now()-pointerAt<700)return;fn()})};bindPager('[data-aw-page-prev]',()=>changePage(-1));bindPager('[data-aw-page-next]',()=>changePage(1));bindPager('[data-aw-zoom-out]',()=>changeZoom(-.15));bindPager('[data-aw-zoom-in]',()=>changeZoom(.15));const pageSelect=qs('[data-aw-page-select]');if(pageSelect)pageSelect.addEventListener('change',()=>{if(!active?.pdfDoc)return;const n=Number(pageSelect.value);if(!Number.isInteger(n))return;active.page=Math.max(1,Math.min(active.pageCount,n));syncViewerControls();renderPaperPage()});${anchor}`;
formal=formal.replace(anchor,direct);
formal+='\n/* v6.13.1.1 iPad-safe paged historical-paper viewer; native page selector + pointer controls; source-review mode retained. */\n';
