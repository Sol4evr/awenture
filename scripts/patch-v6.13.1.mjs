...
const delegated=["if(e.target.closest('[data-aw-page-prev]')){changePage(-1);return}","if(e.target.closest('[data-aw-page-next]')){changePage(1);return}","if(e.target.closest('[data-aw-zoom-out]')){changeZoom(-.15);return}","if(e.target.closest('[data-aw-zoom-in]')){changeZoom(.15);return}"];
...
const pageSelect='<label class="aw-page-select-wrap"><span>Page</span><select data-aw-page-select aria-label="Paper page" disabled>${Array.from({length:p.questionEndPage},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join(\'\')}</select><span data-aw-page-total>of ${p.questionEndPage}</span><strong data-aw-page-label class="aw-page-label-sr">Page 1 of ${p.questionEndPage}</strong></label>';
if(!formal.includes(pageLabel))throw new Error('Missing formal page label markup');
formal=formal.replace(pageLabel,pageSelect);
const nextButton='<button type="button" data-aw-page-next>Next →</button>';
if(!formal.includes(nextButton))throw new Error('Missing formal next-page button');
formal=formal.replace(nextButton,'<button type="button" data-aw-page-next disabled>Next →</button>');

const syncNeedle='const label=qs(\'[data-aw-page-label]\');if(label)label.textContent=`Page ${n} of ${total}`;';
const syncReplacement='const label=qs(\'[data-aw-page-label]\');if(label)label.textContent=`Page ${n} of ${total}`;const select=qs(\'[data-aw-page-select]\');if(select){select.value=String(n);select.disabled=!active.pdfDoc}const totalEl=qs(\'[data-aw-page-total]\');if(totalEl)totalEl.textContent=`of ${total}`;';
...
const navNeedle='if(prev)prev.disabled=n<=1;if(next)next.disabled=n>=total;';
if(!formal.includes(navNeedle))throw new Error('Missing viewer navigation readiness logic');
formal=formal.replace(navNeedle,'if(prev)prev.disabled=!active.pdfDoc||n<=1;if(next)next.disabled=!active.pdfDoc||n>=total;');
...
const syncAnchor='function sync(){transformTests()}';
if(!formal.includes(syncAnchor))throw new Error('Missing sync function anchor');
const pageHandler=`function awSelectPageEvent(e){const select=e.target&&e.target.closest?e.target.closest('[data-aw-page-select]'):null;if(!select||select.disabled||!active?.pdfDoc)return;const n=Number(select.value);if(!Number.isInteger(n))return;const next=Math.max(1,Math.min(active.pageCount,n));if(next===active.page){syncViewerControls();return}active.page=next;syncViewerControls();renderPaperPage()}\nwindow.addEventListener('input',awSelectPageEvent,true);window.addEventListener('change',awSelectPageEvent,true);\n`;
formal=formal.replace(syncAnchor,pageHandler+syncAnchor);
formal+='\n/* v6.13.1.1 iPad-safe historical paper pager; controls enable only after PDF.js document readiness; source-review mode retained. */\n';
fs.writeFileSync(formalPath,formal);

const cssPath=path.join(root,'dist/formal-tests.css');
fs.appendFileSync(cssPath,`\n/* Native iPad page selector — always visible; disabled only while the paper is loading. */\n.aw-page-select-wrap{display:flex;align-items:center;justify-content:center;gap:6px;color:#344054;font-size:13px;font-weight:750;white-space:nowrap}.aw-page-select-wrap select{min-width:58px;min-height:38px;border:1px solid #aeb8c7;border-radius:8px;background:#fff;padding:5px 24px 5px 9px;font:inherit;font-weight:800;color:#243b64}.aw-page-select-wrap select:disabled{opacity:.5;background:#f4f6f8}.aw-page-label-sr{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}@media(max-width:700px){.aw-paper-toolbar{position:sticky;top:0;z-index:5;grid-template-columns:auto minmax(112px,1fr) auto!important}.aw-page-select-wrap{font-size:12px}.aw-page-select-wrap select{min-height:42px;min-width:60px}.aw-paper-swipe-hint{display:none}}\n`);
...
console.log(JSON.stringify({releaseOverlay:'6.13.1.1',historicalPapers:runtime.papers.length,verifiedScoring:runtime.papers.filter(p=>p.scoring==='verified').length,sourceReview:runtime.papers.filter(p=>p.scoring!=='verified').length,pagedViewer:'PDF.js local runtime',pageSelector:'native-select',pagerReadiness:'pdf-document-gated'}));