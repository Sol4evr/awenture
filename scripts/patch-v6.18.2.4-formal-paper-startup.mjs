...
// Use the same native <details>/<summary> structure as later-stage subject categories. The enhancer
// is deliberately coalesced: previous releases scheduled six independent rescans for every Tests
// AND Home click, so repeated iPad navigation accumulated pending DOM work. One animation-frame
// pass plus at most one short retry is enough for the asynchronously rendered historical grid.
...
console.log(JSON.stringify({release:'6.18.2',startupUX:'PASS',y2Accordion:'EXACT_STAGE_DETAILS_PARITY',accordionScheduling:'COALESCED',timerStarts:'AFTER_FIRST_PAGE_VISIBLE',pdfModuleWarmup:'INSTRUCTION_SCREEN',stageViewer:'POINTER_CONTROLS_TOUCH_GESTURES'}));
