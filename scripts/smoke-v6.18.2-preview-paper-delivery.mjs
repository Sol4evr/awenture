...
const p=papers.find(x=>x?.sourcePath?.includes('Digital AB 2010.pdf')&&x?.id&&x?.assetPath)||papers.find(x=>x?.id&&x?.assetPath)||papers[0];
if(!p)throw new Error('no smoke paper available');
const full=await get(`/api/paper?id=${encodeURIComponent(p.id)}`);
requireStatus(full,[200],'valid paper');
...
if(String.fromCharCode(...fullBytes.subarray(0,5))!=='%PDF-')throw new Error('valid paper response is not a PDF');
const delivered=await PDFDocument.load(fullBytes);
const deliveredPages=delivered.getPageCount();
if(deliveredPages!==p.questionEndPage)throw new Error(`learner-only PDF page count mismatch: delivered ${deliveredPages}, approved ${p.questionEndPage}`);
const learnerHeader=Number(full.headers.get('x-aw-paper-learner-pages'));
const sourceHeader=Number(full.headers.get('x-aw-paper-source-pages'));
if(learnerHeader!==p.questionEndPage)throw new Error(`learner page header mismatch: ${learnerHeader}/${p.questionEndPage}`);
if(p.sourcePath?.includes('Digital AB 2010.pdf')&&!(sourceHeader>learnerHeader))throw new Error(`known combined Digital paper was not physically cropped: source ${sourceHeader}, learner ${learnerHeader}`);
...
const rangeHead=new Uint8Array(await ranged.arrayBuffer()).subarray(0,5);
if(String.fromCharCode(...rangeHead)!=='%PDF-')throw new Error('cropped range response is not PDF bytes');
...
console.log(JSON.stringify({release:'6.18.2',previewPaperDeliverySmoke:'PASS',base,paperId:p.id,sourcePath:p.sourcePath,paperStatus:full.status,approvedLearnerPages:p.questionEndPage,deliveredPages,sourcePages:sourceHeader,rangeStatus:ranged.status,unknownIdStatus:missing.status,pathTraversalStatus:traversal.status,protectionBypass:Boolean(bypass)}));
