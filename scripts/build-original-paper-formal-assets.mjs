...
async function rasterDerivative(bytes,indexes,title){
  ...
  for(const index of indexes){
    const sourcePage=await doc.getPage(index+1),base=sourcePage.getViewport({scale:1}),view=sourcePage.getViewport({scale:1.55});
...
console.log(JSON.stringify({historicalFormalAssets:'PASS',questionAssets,answerAssets,vectorPapers,rasterFallbackPapers,rawAnswerPagesExcludedFromTimedFiles:true}));
