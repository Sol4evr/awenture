...
const targets=new Map([
  ['English|2014',5],['English|2016',5],['English|2017',5],['English|2018',6],
  ['Mathematics|2014',5],['Mathematics|2018',6],['Mathematics|2019',99],
  ['Science|2014',6],['Science|2017',5],['Science|2018',5]
]);
...
for(const p of manifest.papers){
  ...
  const count=targets.get(key); const start=count>=doc.numPages?1:Math.max(1,doc.numPages-count+1);
  for(let n=start;n<=doc.numPages;n++){
    const page=await doc.getPage(n); const viewport=page.getViewport({scale:2.15});
    ...
    console.log('AW_OCR_KEY '+JSON.stringify({subject:p.subject,year:p.year,page:n,totalPages:doc.numPages,text:text.replace(/\r/g,'').trim().slice(0,12000)}));
  }
}
