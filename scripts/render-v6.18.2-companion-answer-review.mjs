...
fs.writeFileSync(path.join(outDir,'manifest.json'),JSON.stringify({version:'aw-companion-answer-review-v2',generatedAt:new Date().toISOString(),targets:manifest},null,2)+'\n');
console.log(JSON.stringify({release:'6.18.2',companionAnswerReview:'PASS',version:'v2',targets:manifest.map(x=>({id:x.id,year:x.year,sourceSha256:x.sourceSha256,learnerSha256:x.learnerSha256,totalPages:x.totalPages,pages:x.pages.map(p=>({page:p.page,file:p.file,text:p.text.slice(0,450)}))}))}));
