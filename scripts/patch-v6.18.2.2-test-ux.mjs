...
const PREPARE_DELAYS=[0,50,140,300,650];
...
function schedulePrepare(){for(const d of PREPARE_DELAYS)setTimeout(prepare,d)}
...
document.addEventListener('click',e=>{const h=e.target.closest('.aw-form-heading[data-aw-accordion="1"]');if(h){e.preventDefault();toggle(h);return}if(e.target.closest('[data-a="tests"],[data-a="home"]'))schedulePrepare()});
...
window.addEventListener('awenture:stage-change',schedulePrepare);
window.addEventListener('awenture:progression-ready',schedulePrepare);
schedulePrepare();
})();\n`;
