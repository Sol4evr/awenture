...
const allowedTypes=new Set(['official-year3-answer-sheet-column-end','official-example-test-terminal-question']);
...
for(const r of review.evidence||[]){
  ...
  if(!allowedTypes.has(r.evidenceType))throw new Error(`unapproved reviewed evidence type: ${r.sourcePath}`);
  if(r.evidenceType==='official-year3-answer-sheet-column-end'){
    if(typeof r.sourceUrl!=='string'||!/^https:\/\/(?:www\.)?acara\.edu\.au\//i.test(r.sourceUrl))throw new Error(`official ACARA answer source URL required: ${r.sourcePath}`);
  }else{
    if(r.provider!=='ACARA'||r.questionCount!==35||typeof r.sourceUrl!=='string'||!/^https:\/\/(?:www\.)?nap\.edu\.au\//i.test(r.sourceUrl))throw new Error(`official NAP example evidence invalid: ${r.sourcePath}`);
  }
  const e=qa.papers?.[r.sourcePath];if(!e)throw new Error(`QA entry missing: ${r.sourcePath}`);
  if(e.questionCountVerified){if(e.questionCount!==r.questionCount)throw new Error(`reviewed evidence conflicts with existing verified count: ${r.sourcePath}`);alreadyVerified++;continue}
