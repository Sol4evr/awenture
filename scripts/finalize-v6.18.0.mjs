...
if(catalog.release!=='6.18.0'||catalog.mode!=='stage-linked-source-review')throw new Error('Invalid stage-paper catalog contract');
...
const integrity={release:'6.18.0',baseline:'hardened-v6.16.2',releaseContract:'stage-linked-historical-paper-library-v1',generatedAtBuild:true,fileCount:files.length,files:Object.fromEntries(files.map(rel=>[rel,{bytes:fs.statSync(path.join(dist,rel)).size,sha256:sha256(path.join(dist,rel))}]))};
fs.writeFileSync(path.join(dist,'hardened-integrity.json'),JSON.stringify(integrity,null,2)+'\n');
console.log(JSON.stringify({release:'6.18.0',feature:'stage-linked-historical-paper-library',activated:catalog.summary.activated,pending:catalog.summary.pending,files:files.length,integrityManifest:'PASS'}));
