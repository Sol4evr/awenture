...
test('v6.15.2 serves an upright-normalized 2019 Mathematics paper and renders it in the formal viewer',async({page,request})=>{
  ...
  await page.goto('/');await page.locator('[data-a="tests"]').click();const maths=page.locator('.aw-form-subject').filter({hasText:'Mathematics'});await maths.locator('[data-aw-original-year="2019"]').click();await page.locator('[data-aw-start-original]').click();await expect(page.locator('[data-aw-page-select]')).toBeEnabled({timeout:15000});await expect(page.locator('[data-aw-paper-status]')).toHaveText('',{timeout:15000});const stats=await page.locator('[data-aw-paper-canvas]').evaluate(canvas=>({width:canvas.width,height:canvas.height}));expect(stats.width).toBeGreaterThan(200);expect(stats.height).toBeGreaterThan(200);
