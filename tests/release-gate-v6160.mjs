...
if(!/content="6\.16\.(0|1)/.test(html)||!/RELEASE='6\.16\.(0|1)'/.test(html))throw new Error('v6.16.x release marker missing');
for(const x of ['Weekend program table with day and activities columns','const spatialRecent='])if(!html.includes(x))throw new Error('v6.16.0 regression marker missing '+x);
...
console.log(JSON.stringify({releaseCompatibility:'6.16.x',compatibility:'PASS',spellingBank:spelling.length,answerMix,dailyQuestionCount:15,dailyMix:{English:4,Mathematics:4,Science:4,Spelling:3},audioMode:'WORD_SENTENCE_WORD',parentInsights:'SPELLING_INCLUDED'}));
