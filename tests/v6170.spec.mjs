...
test('learning progression renders one functional QA timeline with all stages temporarily available',async({page})=>{
  await page.goto('/');
  ...
  for(let i=0;i<5;i++)await expect(steps.nth(i)).toBeEnabled();
  await expect(steps.nth(3)).toContainText('ICAS Year 4');
  await expect(steps.nth(4)).toContainText('Opportunity Class');
  const progress=await page.evaluate(()=>JSON.parse(localStorage.getItem('awenture-learning-progression-v1')));
  expect(progress.temporaryUnlockAll).toBe(true);
  expect(progress.unlocked).toEqual(['icas-y2','icas-y3','naplan-y3','icas-y4','oc-prep']);
});

test('temporary QA unlock does not count Parent QA as authentic readiness evidence',async({page})=>{
  ...
  expect(model.progress.unlocked).toEqual(['icas-y2','icas-y3','naplan-y3','icas-y4','oc-prep']);
  expect(model.progress.temporaryUnlockAll).toBe(true);
  expect(model.attempts).toBe(0);
  expect(model.fullPapers).toBe(0);
});
