...
const review=JSON.parse(fs.readFileSync(path.join(root,'quality/stage-paper-question-count-reviewed-source-evidence-v1.json'),'utf8'));
if(review.version!=='aw-stage-paper-reviewed-source-count-evidence-v1')throw new Error('reviewed source-count manifest version mismatch');
const seen=new Set();for(const r of review.evidence||[]){if(seen.has(r.sourcePath))throw new Error(`duplicate reviewed source-count evidence: ${r.sourcePath}`);seen.add(r.sourcePath);const e=qa.papers?.[r.sourcePath];if(!e?.questionCountVerified||e.questionCount!==r.questionCount||e.method!=='reviewed-exact-source-evidence')throw new Error(`reviewed source-count not applied exactly: ${r.sourcePath}`);const abs=path.join(root,r.sourcePath);const sha=crypto.createHash('sha256').update(fs.readFileSync(abs)).digest('hex');if(sha!==r.sha256||e.evidence?.sha256!==r.sha256)throw new Error(`reviewed source-count SHA provenance mismatch: ${r.sourcePath}`);if(e.evidence?.reviewManifest!=='quality/stage-paper-question-count-reviewed-source-evidence-v1.json')throw new Error(`reviewed source-count manifest provenance missing: ${r.sourcePath}`)}
...
console.log(JSON.stringify({release:'6.18.2',reviewedExactSourceCountGate:'PASS',papers:seen.size,naplanSampleNumeracyCount:35,subjectDefaults:'FORBIDDEN',autoScoringImplied:false,progressionCreditImplied:false}));
