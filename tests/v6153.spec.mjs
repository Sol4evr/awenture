...
test('v6.15.3 Daily Practice rescues safe spatial coverage without exceeding visual policy',async({page})=>{
  ...
  const items=await page.evaluate(()=>{const out=[];for(let i=0;i<12;i++){document.querySelector(`[data-q="${i}"]`)?.click();out.push({visual:document.querySelector('.testworkspace')?.classList.contains('visual-question')||false,meta:document.querySelector('.question-meta')?.textContent||'',stem:document.querySelector('.question-pane .q')?.textContent||''});}return out});
  expect(items).toHaveLength(12);expect(items.filter(x=>x.visual).length).toBeGreaterThanOrEqual(3);expect(items.filter(x=>x.visual).length).toBeLessThanOrEqual(4);expect(items.some(x=>x.visual&&x.meta.includes('Space and geometry'))).toBeTruthy();expect(items.every(x=>x.stem.length>0)).toBeTruthy();
});
